 ...
 * @return string
 */
function get_product_type($type, $all = false) {
    $list = array(
        1 => '贷款',
        2 => '房贷',
        3 => '车贷',
...
function get_institution_name($institutionid,$all=false){
    $institutionlist = S('institution_list');
    if(!$institutionlist){
        $institutionlist = M('institution')->WHERE(true)->select();
        S('institution_list',$institutionlist,3600);
    }
    if($all){
